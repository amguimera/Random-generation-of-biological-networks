...
RateConstantScale=0.01; %Scale of interaction speed (this value will then be turned into a distribution to sample from)
...
%Generate species identifiers and calculate the number of interactions in the network
...
for n=1:NumOfNetworks
    ...
    mean=1; %The mean value for the parameter multiplier is 1
    variance=mean/10;
    ...
    [number, MinTimeIndex ] = min( abs( Simulation_Output.Time-EventTime ) );%Identify index closest to the Event time in order not to include peaks/troughs arising from system equilibration
