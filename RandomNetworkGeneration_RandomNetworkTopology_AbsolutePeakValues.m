...
RateConstantScale=0.01; %Scale of speed of interactions. This number will be transformed into a distribution to sample from.
...
FractionPossibleInteractions=0.04; %Fraction of interaction combinations explored

%Generate enzyme identifiers and calculate the number of interactions in the network
...
Interactions=round(FractionPossibleInteractions*InteractionCombinations);
...
for n=1:NumOfNetworks
    ...
    %Create tabulation table with an unchanging uniform probability (this a redundant exercise I didnt change out of lazyness)
    ...
    for i=1:Interactions %For a fraction of the total number of combinations
        ...
        RandomNode=rand(1);
        Rndm_F = datasample(EnzymeIdentifiers,1); %For random model generation

        %Choose a random binary outcome for another random node  
        EdgeType=rand(1);
        RandomNode2=rand(1);
        if EdgeType<=0.5 % 'F' acts on another 'F' (deactivation)
            Rndm_F_2=datasample(EnzymeIdentifiers,1); %For random model generation use sampling with uniform probability with replacement
            ...
            Rndm_E=datasample(EnzymeIdentifiers,1); %For random model generation use sampling with uniform probability with replacement
    ...
    mean=1; %Mean of parameter multiplier is 1
    variance=mean/10;
    ...
    if numel(idxs)>0
        for i=1:numel(idxs)
            idx=idxs(i);
            variation(idx)=RateConstantScale/100; %1 a.u is the minimum rate constant value 
        end
    end
    ...
    [number, MinTimeIndex ] = min( abs( Simulation_Output.Time-EventTime ) );%Identify index closest to event time in order not to include peaks/troughs arising from system equilibration
