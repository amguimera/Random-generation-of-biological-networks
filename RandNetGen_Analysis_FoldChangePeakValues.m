...
for k=1:2:length(Files) %increases in groups of two since files exist in pairs (one for model and one for peak data)
    ...
    for p=1:numel(modelObj.parameters)
        ...
        %Identify index closes to the Event time in order not to include peaks/troughs arising from system equilibration
        ...
        for i=1:numel(StimulatedSpeciesIndex) %for each simulated F species
            ...
            elseif numel(PeakMagnitude)==0 && numel(TroughMagnitude)==0 %no peaks or troughs found
                ResponseChange=0-StimulatedSpeciesMagnitude(i);
                Average_response_data=[Average_response_data; ResponseChange];              
            end
